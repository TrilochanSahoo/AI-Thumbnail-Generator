...
export default function Page() {
  ...
  async function onGenerate() {
    ...
    try {
      console.log("[v0] Generating image with params:", {
        prompt,
        aspect,
        hiRes,
      });
  ...
  const addImageToCanvas = (src: string) =>
    setBoardItems((prev) => [
      ...prev,
      { id: crypto.randomUUID(), type: "image", src, x: 24, y: 24, w: 160 },
    ]);

  const addTextToCanvas = (text: string) =>
    setBoardItems((prev) => [
      ...prev,
      { id: crypto.randomUUID(), type: "text", text, x: 40, y: 40 },
    ]);
      ...
      <div
        className={cn(
          ...
          sidebarOpen && rightOpen
            ? "sm:grid-cols-[240px_minmax(0,1fr)_360px]"
            : sidebarOpen && !rightOpen
      ...
      >
        <aside className='border-r px-4 py-4 md:min-h-dvh'>
          ...
          <nav className='mt-6 flex flex-col gap-1'>
            ...
            <NavItem icon={<FolderHeart size={18} />} label='Library' />
            <NavItem
        ...
        <main className='px-4 py-4 h-dvh flex flex-col'>
          <div
            ref={listRef}
            className='mt-4 flex-1 overflow-y-auto pr-1 space-y-3'
          >
            {messages.map((m) =>
              m.role === "user" ? (
                <motion.div
            ...
            <div className='flex flex-col gap-2'>
              <div className='flex items-start gap-2'>
                <Textarea
                  ...
                  className='min-h-[40px] max-h-40 flex-1'
                  onKeyDown={(e) => {
                  ...
                  draggable
                  onDragStart={(e) =>
                    e.dataTransfer.setData("text/pastel-text", prompt)
                  }
                />
                <Button
              ...
              <Dropzone
                onImages={(urls) => setUploads((prev) => [...prev, ...urls])}
                className='bg-sky-200/30 p-2'
              />
              {uploads.length > 0 && (
                <div className='flex flex-wrap gap-2'>
                  {uploads.map((src, i) => (
                    <div
                      ...
                      draggable
                      onDragStart={(e) =>
                        e.dataTransfer.setData("text/pastel-image-src", src)
                      }
                    ...
                    >
                      ...
                      <div className='absolute inset-0 hidden items-center justify-center gap-1 bg-black/20 p-1 group-hover:flex'>
                        <Button
                          size='sm'
                          variant='secondary'
                          className='bg-rose-200 text-red-500'
                          onClick={() =>
        ...
        <AnimatePresence initial={false}>
          {rightOpen && (
            <motion.section
              ...
              className='border-l px-4 py-4 md:min-h-dvh'
            >
              <RightSettings
    ...
    <div className='flex h-full flex-col gap-4'>
      ...
      <Card className='bg-rose-200/20 border-rose-200'>
        ...
        <CardContent className='text-sm'>
          Use clear subjects, a style, and desired colors: Soft mint-and-sky
          pastel thumbnail for a mindful productivity video, clean typography,
          gentle shadows.
        </CardContent>
      </Card>
      ...
      <Card className='bg-fuchsia-200/20 border-fuchsia-200'>
        <CardHeader>
          <CardTitle className='text-sm'>AI Canvas editor</CardTitle>
        </CardHeader>
